...
**Root cause:** the 2.0.77 sprite sheet (generated by upstream `factoriodump`) has the pump at grid position (col 3, row 5) — but the dataset's `sprites.extra.slot_icon_module` block still pointed there from the prior 2.0.55 layout, and the dataset also re-used the same coordinates for the `empty-module-slot` item. The sprite-sheet content had moved; the dataset coordinates had not been updated to match. The Space Age 2.0.55 dataset that we forked from has the same bug — this was inherited, not introduced.

**Fix:** painted a proper empty-slot placeholder tile into an unused position on the sprite sheet (col 29, row 29) and updated both `sprites.extra.slot_icon_module` and the `empty-module-slot` item entry to point there. New sprite-sheet MD5 hash rolled forward in the dataset; old file removed.

Validated with: dataset validator (clean), 118 unit tests (all pass), smoke test (clean), and visual check on the live deployment.
